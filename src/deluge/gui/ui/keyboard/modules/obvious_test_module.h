#pragma once

#include "gui/ui/keyboard/modules/module_base.h"

namespace deluge::gui::ui::keyboard::modules {

/// @brief Super obvious test module
/// @details Fills entire screen with bright colors to test if modules work
class ObviousTestModule : public KeyboardModule {
public:
	ObviousTestModule();
	~ObviousTestModule() override = default;

	// Module interface implementation
	bool init() override;
	void update(uint32_t deltaTime) override;
	void render(RGB image[][kDisplayWidth + kSideBarWidth],
	            uint8_t occupancyMask[][kDisplayWidth + kSideBarWidth]) override;
	bool handlePadPress(int32_t x, int32_t y, int32_t velocity) override;
	bool handleEncoder(int32_t offset, bool isVertical) override;

	// Module metadata
	const char* getName() const override { return "OBVIOUS TEST"; }
	const char* getAuthor() const override { return "Deluge Modules"; }
	const char* getVersion() const override { return "1.0.0"; }
	const char* getDescription() const override { return "Super obvious test to see if modules work"; }

private:
	uint32_t animationFrame_ = 0;
};

} // namespace deluge::gui::ui::keyboard::modules
