...
bool StepSequencerMode::handleSelectEncoder(int32_t offset) {
	...
	if (shouldEditProbability) {
		...
		steps_[heldPadX_].probability = static_cast<uint8_t>(SequencerMode::clampValue(
...
bool StepSequencerMode::handleHorizontalEncoder(int32_t offset, bool encoderPressed) {
	...
	if (!Buttons::isShiftButtonPressed() && isNotePadHeld()) {
		...
		steps_[heldPadX_].velocity = static_cast<uint8_t>(
...
bool StepSequencerMode::handleModeSpecificVerticalEncoder(int32_t offset) {
	...
	if (Buttons::isButtonPressed(deluge::hid::button::Y_ENC)) {
		...
		if (isNotePadHeld()) {
			...
			steps_[heldPadX_].gateLength = static_cast<uint8_t>(
		...
		for (int32_t i = 0; i < kNumSteps; i++) {
			...
			steps_[i].octave = static_cast<int8_t>(
			    SequencerMode::clampValue(newOctave, static_cast<int32_t>(-3), static_cast<int32_t>(3)));
		}
		...
		for (int32_t y = 0; y < kDisplayHeight; y++) {
			...
			// y3-y7: Note pads (5 notes with scrolling)
				...
				if (actualNoteIndex < numScaleNotes_ && actualNoteIndex == step.noteIndex) {
					...
					int32_t velocityBrightness =
					    65 + static_cast<int32_t>(step.velocity) + (static_cast<int32_t>(step.velocity) / 2);
...
int32_t StepSequencerMode::processPlayback(void* modelStackPtr, int32_t absolutePlaybackPos) {
	...
	if (absolutePlaybackPos > lastAbsolutePlaybackPos_) {
		...
		else if (effects.direction != 0 && effects.direction != 1 && oldStep == numActiveSteps_ - 1
		         && currentStep_ == 0) {
			repeatCount_++;
		}
	}
...
void StepSequencerMode::advanceStep(int32_t direction) {
	...
	case 0: // Forward
		currentStep_ =
		    static_cast<uint8_t>((static_cast<int32_t>(currentStep_) + 1) % static_cast<int32_t>(numActiveSteps_));
		break;
